...
-- (askNoErrs m) runs m
-- If m fails, (askNoErrors m) fails
-- If m succeeds with result r, (askNoErrors m) succeeds with result (r, b),
...
{-
-- Simon's proposed implementation. Does not work as intended
checkNoErrs m = do { (r, no_errs) <- askNoErrs m
                   ; if no_errs then return r else failM }
-}
checkNoErrs main
  = do  { (msgs, mb_res) <- tryTcLIE main
        ; addMessages msgs
