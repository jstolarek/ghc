...
import FunDeps
import FamInstEnv  ( isDominatedBy, injectiveBranches, InjectivityCheckResult(..) )
    ...
    gather_conflicts inj prev_branches cur_branch (acc, n) branch
      ...
      = case injectiveBranches inj cur_branch branch of
          InjectivityUnified ax1 ax2
            | ax1 `isDominatedBy` (replace_br prev_branches n ax2) -> (acc, n + 1)
            | otherwise                                            -> (branch : acc, n + 1)
