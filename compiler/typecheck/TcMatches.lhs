...
tcGuardStmt _ (BodyStmt guard _ _) res_ty thing_inside
  = do	{ guard' <- tcMonoExpr guard boolTy
	...
	; return (BodyStmt guard' noSyntaxExpr noSyntaxExpr, thing) }
...
tcLcStmt _ _ (BodyStmt rhs _ _) elt_ty thing_inside
  = do	{ rhs'  <- tcMonoExpr rhs boolTy
	...
	; return (BodyStmt rhs' noSyntaxExpr noSyntaxExpr, thing) }
...
--
tcMcStmt _ (BodyStmt rhs then_op guard_op) res_ty thing_inside
  = do	{ -- Deal with rebindable syntax:
	...
	; return (BodyStmt rhs' then_op' guard_op', thing) }
...
tcDoStmt _ (BodyStmt rhs then_op _) res_ty thing_inside
  = do	{   	-- Deal with rebindable syntax; 
	...
	; then_op' <- tcSyntaxOp DoOrigin then_op 
        ...
        ; rhs' <- tcMonoExprNC rhs rhs_ty
	; thing <- thing_inside new_res_ty
	; return (BodyStmt rhs' then_op' noSyntaxExpr, thing) }
