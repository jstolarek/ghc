...
dataflowPassFwd (CmmGraph {g_entry=entry, g_graph=graph}) facts fwd = do
  (graph, facts, NothingO) <- analyzeAndRewriteFwd fwd graph (mkFactBase (fp_lattice fwd) facts)
...
dataflowAnalFwd :: NonLocal n =>
                ...
                -> FwdPass UniqSM n f
                -> BlockEnv f
dataflowAnalFwd (CmmGraph {g_graph=graph}) facts fwd =
  analyzeFwd fwd graph (mkFactBase (fp_lattice fwd) facts)
