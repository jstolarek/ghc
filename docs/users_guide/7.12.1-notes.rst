...
- ``Template Haskell`` has now explicit support for representing GADTs.  Until
  ...
  are now supported during quoting and splicing.  However, GADTs are not
  supported during reification: reified GADT constructors will still be encoded
  using ``NormalC``, ``RecC`` and ``ForallC``.
